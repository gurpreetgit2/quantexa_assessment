...
/** * A common problem we face at Quantexa is having lots of disjointed raw
  * sources of data and having to aggregate and collect relevant pieces of
  * information into hierarchical case classes which we refer to as Documents.
  * This exercise simplifies the realities of this with just two sources of high
  * quality data however reflects the types of transformations we must perform.
  *
  * You have been given customerData and accountData. This has been read into a
  * DataFrame for you and then converted into a Dataset of the given case
  * classes.
  *
  * If you run this App you will see the top 20 lines of the Datasets provided
  * printed to console
  *
  * This allows you to use the Dataset API which includes
  * .map/.groupByKey/.joinWith ect. But a Dataset also includes all of the
  * DataFrame functionality allowing you to use .join ("left-outer","inner" ect)
  *
  * https://spark.apache.org/docs/latest/sql-programming-guide.html
  *
  * The challenge here is to group, aggregate, join and map the customer and
  * account data given into the hierarchical case class customerAccountoutput.
  * We would prefer you to write using the Scala functions instead of
  * spark.sql() if you choose to perform Dataframe transformations. We also
  * prefer the use of the Datasets API.
  *
...
// ****************************** BEGIN SOLUTION BLOCK ******************************

  val customerAccountDS = customerDS.joinWith(
    accountDS,
    customerDS("customerId") === accountDS("customerId"),
    "left_outer"
  )

  val groupedDS = customerAccountDS.groupByKey { case (customer, _) =>
    customer
  }
